...
export const updateTest = async function (_idTest: number, _name: string, _description: string, _category: string, _userEmail: string, callback: callback) {
  const sql = 'UPDATE test SET name = ?, description = ?, category = ?, userEmail = ? WHERE idTest = ?'
  conn.query(sql, [_name, _description, _category, _userEmail, _idTest], callback)
}

// Actualizar una pregunta existente
export const updateAsk = async function (_idAsk: number, _ask: string, _answer1: string, _answer2: string, _answer3: string, _answer4: string, _sol: string, _multi: string, _image: string, _reference: string, callback: callback) {
